...
- Validation sweep scripts accept `--solver`, `--stride` and `--tag`, and `validation/compare_solvers.py` writes a per-order deviation table and a side-by-side plot for each case. Running a sweep with no flags reproduces its previous behaviour and output paths.
