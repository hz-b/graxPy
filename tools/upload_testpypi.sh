...
if [[ -f "${TOKEN_FILE}" ]]; then
    TOKEN_VALUE="$(get_named_token "${TOKEN_FILE}" "TestPyPI")"
    if [[ -z "${TOKEN_VALUE}" ]]; then
        echo "Error: could not find '# TestPyPI token' entry in ${TOKEN_FILE}."
        exit 1
    fi
    echo "Using TestPyPI token from ${TOKEN_FILE} (non-interactive upload)."
    export TWINE_USERNAME="__token__"
    export TWINE_PASSWORD="${TOKEN_VALUE}"
    "${PYTHON_BIN}" -m twine upload --non-interactive --repository testpypi dist/*
else
...
echo "============================================================"
echo
echo "Install test command:"
echo
echo "pip install --index-url https://test.pypi.org/simple/ \\"
echo "    --extra-index-url https://pypi.org/simple graxpy"
