...
if [[ "${SKIP_IMAGE_SYNC}" == false ]]; then
    ...
    # Copy the roughness comparison plot generated by the fixed_angle_roughness example.
    if [[ -f "${PROJECT_ROOT}/examples/simulation/fixed_angle_roughness/results/fixed_angle_roughness_order1_comparison.png" ]]; then
      ...
      echo "Warning: roughness comparison image not found; keeping the checked-in docs image."
    fi
