...
if [[ "${EXPLICIT_BUILD_SELECTION}" == false ]]; then
    if [[ "${LAUNCH_HTML}" == true ]]; then
        ...
        BUILD_HTML=true
        if command -v pdflatex >/dev/null 2>&1 && command -v make >/dev/null 2>&1; then
            BUILD_LATEX=true
            BUILD_PDF=true
        else
            echo "Warning: pdflatex and/or make are unavailable; building HTML only."
            echo "Install a LaTeX distribution and rerun with --pdf to build the PDF."
        fi
    fi
fi

if [[ "${BUILD_PDF}" == true ]] && ! command -v pdflatex >/dev/null 2>&1; then
    echo "Error: --pdf requires pdflatex. Install a LaTeX distribution first."
    exit 1
fi

if [[ "${BUILD_PDF}" == true ]] && ! command -v make >/dev/null 2>&1; then
    echo "Error: --pdf requires make. Install make first."
    exit 1
fi
...
if [[ "${SKIP_IMAGE_SYNC}" == false ]]; then
    ...
    cp "${PROJECT_ROOT}/examples/simulation/batch_user_cases/results/batch_user_cases_orders_1_3_vs_depth_rcwa.png" \
      "${SIM_IMAGE_DIR}/batch_user_cases_orders_1_3_vs_depth.png"
    cp "${PROJECT_ROOT}/examples/simulation/fixed_angle_sweep/results/fixed_angle_orders_1_3_rcwa.png" \
      "${SIM_IMAGE_DIR}/fixed_angle_orders_1_3.png"
    cp "${PROJECT_ROOT}/examples/simulation/fixed_angle_roughness/plots_roughness_kind_comparison/roughness_kind_comparison_order1_comparison.png" \
      "${SIM_IMAGE_DIR}/fixed_angle_roughness_order1_comparison.png"
    cp "${PROJECT_ROOT}/examples/simulation/monochromator_sweep/results/monochromator_orders_1_3_rcwa.png" \
      "${SIM_IMAGE_DIR}/monochromator_orders_1_3.png"
    cp "${PROJECT_ROOT}/examples/simulation/energy_angle_sweep/results/energy_angle_multilayer_fast_rcwa.png" \
    ...
    cp "${PROJECT_ROOT}/examples/simulation/blazed_multilayer_sweep/results/blazed_multilayer_all_orders_rcwa.csv" \
    ...
    cp "${PROJECT_ROOT}/examples/simulation/deep_grating_limits/results/deep_grating_limits_p.png" \
      "${SIM_IMAGE_DIR}/deep_grating_limits_p.png"
    cp "${PROJECT_ROOT}/examples/simulation/continuous_vs_staircase/results/continuous_vs_staircase_p.png" \
      "${SIM_IMAGE_DIR}/continuous_vs_staircase_p.png"
    cp "${PROJECT_ROOT}/examples/simulation/solver_runtime/results/solver_runtime_full.png" \
      "${SIM_IMAGE_DIR}/solver_runtime_full.png"
    echo
fi
