"""Ax optimization for the laminar comparison grating with roughness fitting."""
...
config = LaminarAxConfig(
    ...
    output_dir=example_root / "results" / "laminar_discrete_fit_with_roughness",
    angle_mode="fixed",
    ...
    period_lpermm_bounds=ParameterBounds(395.0, 405.0),
    width_to_period_ratio_bounds=ParameterBounds(0.50, 0.85),
    ...
    roughness_sigma_nm_bounds=ParameterBounds(0.2, 1.0),
    experiment_name="laminar_discrete_fit_with_roughness",
    loss_name="mse",
    save_best_fit_plot=True,
    evaluation_energies_ev=[100.0, 300.0, 600.0],
)
