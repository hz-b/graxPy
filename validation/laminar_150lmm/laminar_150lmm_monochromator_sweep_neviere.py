...
Thin entry point over ``laminar_150lmm_monochromator_sweep.py``. The grating geometry, energy grid,
...
``comparison_laminar_150lmm_monochromator.py`` can overlay both.

Any extra arguments are forwarded, so ``--stride`` and the sweep's own flags
still work:

```bash
python laminar_150lmm/laminar_150lmm_monochromator_sweep_neviere.py
python laminar_150lmm/laminar_150lmm_monochromator_sweep_neviere.py --stride 10
```
"""
...
SWEEP_SCRIPT = Path(__file__).resolve().parent / "laminar_150lmm_monochromator_sweep.py"
