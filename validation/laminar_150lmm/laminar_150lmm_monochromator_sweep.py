...
args = add_solver_arguments(
    argparse.ArgumentParser(description="Laminar 150 l/mm monochromator sweep")
).parse_args()

grax.setup_logging(level="INFO", run_id=f"laminar_150lmm_monochromator_{args.solver}")
...
csv_path = solver_output_path(
    results_dir / "laminar_150lmm_monochromator_all_orders.csv", args.solver, args.tag
)
orders_plot_path = solver_output_path(
    results_dir / "laminar_150lmm_monochromator_orders_1_3.png", args.solver, args.tag
)
...
energies_ev = apply_stride(np.arange(10.0, 1000.1, 2.0), args.stride)
