"""Grating and sweep definition for the laminar 150 l/mm validation case.
...
QUICK_ENERGIES_EV = np.asarray([100.0, 500.0, 900.0], dtype=float)
...
def build_grating(*, quick: bool = False) -> grax.LaminarGrating:
    ...
    return grax.LaminarGrating(
        ...
        layer_material=load_optical_constants("Au"),
        layer_thickness_nm=LAYER_THICKNESS_NM,
        top_cap_material=None,
        top_cap_thickness_nm=0.0,
        x_resolution_nm=2.0 if quick else 0.3,
        z_resolution_nm=2.0 if quick else 0.3,
    )
...
def build_energies_ev(*, quick: bool = False, stride: int = 1) -> np.ndarray:
    ...
    Matches ``RR-test-lGR150-gd60-nonSP-20_dm_de_r_m1_single.dat``: 10 eV to
    1000 eV in 2 eV steps.
    ...
    energies = QUICK_ENERGIES_EV if quick else np.arange(10.0, 1000.1, 2.0)
...
def build_cases(*, quick: bool = False, stride: int = 1):
    ...
    return grax.monochromator_cases(
...
def sweep_metadata(solver: str) -> dict[str, object]:
    """Return checkpoint metadata describing this sweep.

    Args:
        solver: Solver used for the run.

    Returns:
        JSON-serializable metadata mapping.
    """
...
def output_paths(solver: str) -> dict[str, Path]:
    ...
    return {
        "all_orders_csv": RESULTS_DIR / f"laminar_150lmm_monochromator_all_orders_{solver}.csv",
        "orders_plot": RESULTS_DIR / f"laminar_150lmm_monochromator_orders_1_3_{solver}.png",
        "profile_plot": RESULTS_DIR / "laminar_150lmm_monochromator_profile.png",
    }
