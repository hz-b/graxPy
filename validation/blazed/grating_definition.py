"""Grating and sweep definition for the blazed 600 l/mm validation case.
...
QUICK_ENERGIES_EV = np.asarray([100.0, 500.0, 1000.0], dtype=float)
...
def build_grating(*, quick: bool = False) -> grax.BlazedGrating:
    """Return the blazed grating for this validation case.
...
def build_energies_ev(*, quick: bool = False, stride: int = 1) -> np.ndarray:
    ...
    energies = QUICK_ENERGIES_EV if quick else np.arange(50.0, 2000.0, 10.0)
...
def output_paths(solver: str) -> dict[str, Path]:
    ...
    return {
        "all_orders_csv": RESULTS_DIR / f"{stem}_{solver}.csv",
        "orders_plot": RESULTS_DIR / f"{stem}_{solver}.png",
        "profile_plot": RESULTS_DIR / "blazed_comparison_profile.png",
