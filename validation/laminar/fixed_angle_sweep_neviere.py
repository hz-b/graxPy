...
Thin entry point over ``fixed_angle_sweep.py``. The grating geometry, energy grid,
...
``comparison.py`` can overlay both.

Any extra arguments are forwarded, so ``--stride`` and the sweep's own flags
still work:

```bash
python laminar/fixed_angle_sweep_neviere.py
python laminar/fixed_angle_sweep_neviere.py --stride 10
```
"""
...
SWEEP_SCRIPT = Path(__file__).resolve().parent / "fixed_angle_sweep.py"
